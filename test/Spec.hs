...
opossumSpec = do
  describe "Opossum Model" $ let 
      ...
      otherResources = Opossum_Resources (Map.singleton "root" (Opossum_Resources (Map.singleton "other" (Opossum_Resources Map.empty (Set.singleton "file4"))) Set.empty)) (Set.singleton "file5")
      allResourcesSerialized = "{\"root\":{\"other\":{\"file4\":1},\"subfolder\":{\"file1\":1,\"file2\":1},\"file3\":1},\"file5\":1}"
    in do
      it "testFolderAndFileMerging" $ do
        (fpToResources True "path/to/file") <> (fpToResources False "path/other/dir") `shouldBe` 
          Opossum_Resources ( Map.singleton "path" 
                                        (Opossum_Resources ( Map.fromList [("to", Opossum_Resources Map.empty (Set.singleton "file"))
                                                                      ,("other", Opossum_Resources (Map.singleton "dir" mempty) Set.empty)]
                                                      )
                                                      Set.empty)
                        )
                        Set.empty
        countFiles ((fpToResources True "path/to/file") <> (fpToResources False "path/other/dir")) `shouldBe` 1
  ...
  describe "Opossum Model deserialization" $ let
    ...
    in do
      ...
      it "parsing of EA works, case 1" $ do
        let ea_str = B.fromStrict $ C.unlines
            ...
            expected_coordinates = Opossum_Coordinates Nothing Nothing (Just "adduser") (Just "3.118")
            expected_ea = Opossum_ExternalAttribution
              ...
              Nothing
              False
        ea <- case (A.eitherDecode ea_str :: Either String Opossum_ExternalAttribution) of 
              Right ea' -> return ea'
              Left err -> fail err
  ...
  describe "Opossum Utils" $ let
      source = Opossum_ExternalAttribution_Source "test" 100
      expected_coordinates version = Opossum_Coordinates Nothing Nothing (Just "name") (Just version)
      ea1 = Opossum_ExternalAttribution source 100 Nothing Nothing (expected_coordinates "1.2") Nothing Nothing Nothing False
      ea2 = Opossum_ExternalAttribution source 100 Nothing Nothing (expected_coordinates "1.3") Nothing Nothing Nothing False
      ea3 = Opossum_ExternalAttribution source 100 Nothing Nothing (expected_coordinates "1.2") Nothing Nothing Nothing True
