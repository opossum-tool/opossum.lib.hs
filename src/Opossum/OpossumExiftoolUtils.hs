...
findFirstForKeys keys o = Maybe.listToMaybe $ Maybe.mapMaybe
  (\f -> case f o of
    Just (A.String t) ->
      let tStrip = T.strip t in if T.null tStrip then Nothing else Just tStrip
    _ -> Nothing
  )
  (map HM.lookup keys)
...
exiftoolEntryToEA o =
  ...
  in  if isNotEmpty
        then Just $ ExternalAttribution source
                                        90
                                        comment
                                        Nothing
                                        coordinates
...
exiftoolEntryToOpossum (fp, o) = do
  ...
  return $ case exiftoolEntryToEA o of
    Just ea ->
      (mempty { _resources               = fpToResources True fp
